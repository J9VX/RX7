            ...
            try:
                ...
                for entry in playlist_info["entries"]:
                    ...
                    info = {
                        ...
                        "duration_min": time_to_seconds(duration_sec),
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(api_url, params=params) as response:
                ...
                return {
                    ...
                    "duration_min": time_to_seconds(info.get("duration", 0)),
                    "thumb": thumb_path,
...
class YouTubeAPI:
    ...
    async def details(self, link: str, videoid: Union[bool, str] = None):
        ...
        # Check if it's a Saavn link
        if await self.saavn.valid(link):
            info = await self.saavn.info(link)
            return (
                info["title"],
                info["duration_min"],
                info["duration_sec"],
    ...
    async def playlist(self, link, limit, user_id, videoid: Union[bool, str] = None):
        ...
        if await self.saavn.valid(link) and await self.saavn.is_playlist(link):
            playlist = await self.saavn.playlist(link, limit)
            return [song["url"] for song in playlist]
    ...
    async def slider(self, link: str, query_type: int, videoid: Union[bool, str] = None):
        ...
        if await self.saavn.valid(link):
            info = await self.saavn.info(link)
            return info["title"], info["duration_min"], info["thumb"], info["_id"]
    ...
    async def download(self, link: str, mystic, video: Union[bool, str] = None, 
        ...
        # Handle Saavn downloads
        if await self.saavn.valid(link):
            if songaudio or songvideo:
                file_path, details = await self.saavn.download(link)
